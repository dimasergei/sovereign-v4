...
async fn main() -> Result<()> {
    ...
    match broker.get_account().await {
        Ok(account) => {
            info!("Alpaca Account: ${} equity, ${} cash", account.equity, account.cash);
        }
        Err(e) => {
    ...
    info!("Strategy: min_conviction={}, risk_reward=1:{}", cfg.strategy.min_conviction, cfg.strategy.risk_reward_ratio);
    info!("Guardian: {}", coordinator.guardian_status());
    info!("Broker: Alpaca (paper={})", cfg.broker.paper);
    info!("Health: Monitoring enabled (30s tick timeout, 6m candle timeout)");
        ...
        if last_health_check.elapsed().as_secs() >= 10 {
            ...
            match health.check() {
                ...
                HealthStatus::StaleData { seconds, gaps } => {
                    warn!("HEALTH: No ticks for {}s (gap #{})", seconds, gaps);
                    
                    if health.should_alert() && telegram_enabled {
                        let _ = telegram::send(&format!(
                            "⚠️ Data gap detected: {}s without ticks (gap #{})",
                            seconds, gaps
                        )).await;
                    }
                }
